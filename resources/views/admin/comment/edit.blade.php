...
<div class="container" >
  <form action="{{ route('comment.update', $data->id) }}" method="POST" style="width: 100%" enctype="multipart/form-data">
    @csrf @method('PUT')
    <div class="row">
      <div class="col-md-9">
        ...
        @else
          <div class="form-group" >
            <label for="my-select">Comment</label>
            <select id="my-select" class="form-control" name="cmt_id">
              <option value="{{ $model->replyable->id }}" selected>
                {{ strip_tags($model->replyable->body) }}
              </option>
            </select>
          </div>
        @endif
